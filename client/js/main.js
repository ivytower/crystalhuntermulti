...
var GameState = {
    ...
    create: function () {
       ...
       /* this.ship = this.game.add.sprite(this.game.world.centerX, this.game.world.centerY, 'ship');
        ...
        // Camera follows ship
       // this.game.camera.follow(new Pointer());
        ...
        // --- ASTEROID SPAWNS
        socket.on('getasteroids',makeasteroids);
        /*for (var i = 0; i < 50; i++) {
       ...
       // asteroids.forEachAlive(emitasteroiddata,this);
        //socket.on('getasteroids',getasteroids);
    ...
    update: function () {
        ...
        if (this.game.physics.arcade.distanceToPointer(this.ship) > 50) {
            ...
            console.log(this.ship.centerX+'  '+this.ship.centerY);
            var movement={
...
var emitasteroiddata=function(sprite){
    //console.log(sprite.centerX+'  '+sprite.centerY);
    var movement={

        x:sprite.centerX,
        y:sprite.centerY
    }
    socket.emit('sprite',movement);
};
...
var makeasteroids=function(data){
    //Math.floor(Math.random() * 5)
    for (var i = 0; i < 50; i++) {
        var astKey = ['bigBlueAst', 'bigRedAst', 'bigGreyAst', 'bigWhiteAst', 'bigBrownAst'][data[i].key];
        var ast = new Asteroid(game, data[i].x, data[i].y, astKey, BIG_AST_SCALE, BIG_AST_HEALTH, BIG_AST_SPEED);
        ast.body.bounce.set(0.8);
        asteroids.add(ast);
        console.log(ast.centerX,ast.centerY);
    }
